...
public abstract class ConnectMqttCallback implements IPipeCallback {
    ...
    public void onError(final int status, final IPipeItem pipeItem) {
        if (pipeItem != null) {
            ...
            onFailed(status, pipeItem.getErrorMessage());
        }
    }

    abstract public void onResultReceived(MqttItem mqttItem);

    abstract public void onFailed(int status, String message);
}